...
export class ProductoService {
  ...
  protected convertDateFromClient(producto: IProducto): IProducto {
    return Object.assign({}, producto, {
      fechaRegistro: producto.fechaRegistro?.isValid() ? producto.fechaRegistro.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((producto: IProducto) => {
        producto.fechaRegistro = producto.fechaRegistro ? dayjs(producto.fechaRegistro) : undefined;
      });
    }
    return res;
  }
